...
public class ConverterSvcTest {
    ...
    private InputStream gbpStream;
    private InputStream eurStream;
    ...
    public void setUp() {
        ...
        stream = new ByteArrayInputStream("{\"data\":{\"amount\":\"11486.5341\",\"base\":\"BTC\",\"currency\":\"USD\"}}".getBytes());
        gbpStream = new ByteArrayInputStream("{\"data\":{\"amount\":\"8900.8693\",\"base\":\"BTC\",\"currency\":\"GBP\"}}".getBytes());
        eurStream = new ByteArrayInputStream("{\"data\":{\"amount\":\"9809.3278\",\"base\":\"BTC\",\"currency\":\"EUR\"}}".getBytes());
    ...
    public void getExchangeRate_GBP_ReturnsGBPExchangeRate() throws IOException {
        ...
        when(response.getStatusLine()).thenReturn(statusLine);
        when(response.getEntity()).thenReturn(entity);
        when(entity.getContent()).thenReturn(gbpStream);
        when(client.execute(any(HttpGet.class))).thenReturn(response);
    ...
    public void getExchangeRate_EUR_ReturnsEURExchangeRate() throws IOException {
        ...
        when(response.getStatusLine()).thenReturn(statusLine);
        when(response.getEntity()).thenReturn(entity);
        when(entity.getContent()).thenReturn(eurStream);
        when(client.execute(any(HttpGet.class))).thenReturn(response);
